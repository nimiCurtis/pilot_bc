...
class ObservationTransform:
    ...
    def _define_transforms(self):
        ...
        random_crop = transforms.RandomResizedCrop(size=(self.width, self.height),
                                ...
                                antialias=True)
        random_crop = transforms.RandomApply(transforms=[random_crop], p=0.15)
        
        # Brightness adjustment transform with a probability of 0.1
        brightness_adjustment = transforms.ColorJitter(brightness=0.3)  # Adjust brightness factor
        brightness_adjustment = transforms.RandomApply(transforms=[brightness_adjustment], p=0.25)
