        ...
        if robot_number_of_bags>0:
            ...
            for folder_name in test_folder_names + train_folder_names:
                ...
                with open(os.path.join(args.data_dir,folder_name, "metadata.json"), 'r') as file:
                    ...
                    ## TODO: add list of x and y
                    max_lin_vel.append([stats["dx"]["max"],stats["dy"]["max"]])
                    min_lin_vel.append([stats["dx"]["min"],stats["dy"]["min"]])
                    mean_lin_vel.append([stats["dx"]["mean"],stats["dy"]["mean"]])
                    std_lin_vel.append([stats["dx"]["std"],stats["dy"]["std"]])
            ...
            tot_max_lin_vel = np.max(max_lin_vel,axis=0)
            tot_min_lin_vel = np.min(min_lin_vel,axis=0)
            tot_mean_lin_vel = np.mean(mean_lin_vel,axis=0)
            tot_std_lin_vel = np.mean(std_lin_vel,axis=0)
            ...
            dataset_config = recursive_update(d=dataset_config, u = {robot_name:   
                                            {"stats" : {"max_lin_vel": [float(x) for x in np.round(tot_max_lin_vel,4)],
                                            "min_lin_vel": [float(x) for x in np.round(tot_min_lin_vel,4)],
                                            "mean_lin_vel": [float(x) for x in np.round(tot_mean_lin_vel,4)],
                                            "std_lin_vel": [float(x) for x in np.round(tot_std_lin_vel,4)],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-dir", "-i", help="Directory containing the data",default="/home/roblab20/dev/pilot/pilot_bc/pilot_dataset/pilot_target_tracking", required=False
    )
    
    parser.add_argument(
        "--dataset-name", "-r", help="Name of the dataset",default="pilot_target_tracking",  required=False, 
    )
