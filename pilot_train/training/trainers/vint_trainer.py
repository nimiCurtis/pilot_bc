...
class ViNTTrainer(BasicTrainer):
    """
    ...
    def train_one_epoch(self, epoch: int)->None:
        ...
        for i, data in enumerate(tqdm_iter):
            ...
            final_encoded_condition = self.model("decoder",
                                tokens=tokens)

            vint_output = self.model("action_pred",
    ...
    def evaluate_one_epoch(self, dataloader:DataLoader, eval_type:str, epoch:int)->Tuple:
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(tqdm_iter):
                ...
                tokens = torch.cat((fused_modalities_encoding, goal_encoding), dim=1)  # >> Concat the lin_encoding as a token too
                final_encoded_condition = eval_model("decoder",
                                    tokens=tokens)

                vint_output = eval_model("action_pred",
                                        final_encoded_condition=final_encoded_condition)
