...
class ViT(BaseModel):
    def __init__(
        self,
        vision_encoder_config, data_config
    ) -> None:
        """
        ViT class
        """
        ...
        mha_num_attention_heads = vision_encoder_config.mha_num_attention_heads
        mha_num_attention_layers = vision_encoder_config.mha_num_attention_layers
        
        super(ViT, self).__init__(in_channels=vision_encoder_config.in_channels,
                                pretrained=vision_encoder_config.pretrained)
    ...
    def extract_featurs(
        ...
        x = torch.cat(obs_img_list, dim=-1)
        ...
        assert x.shape[3] == self.image_width*(self.context_size), f"input image width is not {self.image_width}*(context_size + 2)"
...
## TODO: check vit implementation for sequence and small datasets
