...
class ViTExtractor:
    ...
    def extract_out_features(self, batch: torch.Tensor, layer: List[int],
                             load_size: Union[int, Tuple[int, int]] = None,
                              facet: str = 'key', bin: bool = False,  ) -> torch.Tensor:
        """
        verify that input is :
                img = torch.from_numpy(img).to(self.device).float() / 255.0
                img = torch.permute(img, (2, 0, 1))
                torch.stack(img_batch) -> i.e [BxCxHxW]
        Args:
            batch:
              extract descriptors from the model
        :param batch: batch to extract descriptors for. Has shape BxCxHxW.
        ...
        img_batch = self.preprocess(batch, load_size)[0]
        if len(img_batch.shape) == 3: # if 1 image
            img_batch = img_batch.unsqueeze(0)

        for image in img_batch:
            # image needs to be resized s.t. H, W are divisible by dino_stride
            if "dinov2" in self.model_type:
                image = transforms.Resize(
                    (
                        (image.shape[1] // self.stride) * self.stride,
                        (image.shape[2] // self.stride) * self.stride,
                    )
                )(image)
            with torch.no_grad():
